		...
		public:
			...
			GINL GAIA::GVOID push(const _DataType& t)
			{
				if(m_size == m_capacity)
				{
					...
					_SizeType newsize = increaser.Increase(m_capacity);
					_DataType* pTemp = new _DataType[newsize];
					if(m_pData != GNULL)
					{
						for(_SizeType c = 0; c < m_size; ++c)
							pTemp[c] = m_pData[c];
						delete[] m_pData;
					}
					m_pData = pTemp;
			...
			GINL GAIA::BL pop(){if(m_size > 0){--m_size; return GAIA::True;} return GAIA::False;}
			GINL const _DataType& top() const{return m_pData[m_size];}
			GINL _DataType& top(){return m_pData[m_size - 1];}
