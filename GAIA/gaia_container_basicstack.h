...
namespace GAIA
{
	namespace CONTAINER
	{
		...
		{
		public:
			BasicStack(){m_pData = NULL; m_size = m_cursize = 0;}
			...
			GINL GAIA::VOID reserve(const _SizeType& size){this->destroy();if(size > 0){m_pData = new _DataType[size];m_size = size;m_cursize = 0;}}
			GINL GAIA::VOID destroy(){if(m_pData != NULL){delete[] m_pData; m_size = m_cursize = 0;}}
			GINL GAIA::VOID push(const _DataType& t)
			{
				if(m_cursize == m_size)
				{
