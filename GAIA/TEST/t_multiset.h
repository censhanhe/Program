#ifndef		__T_MULTISET_H__
#define		__T_MULTISET_H__

namespace GAIATEST
{
	GINL GAIA::N32 t_multiset(GAIA::FILESYSTEM::File& file, GAIA::PRINT::Print& prt)
	{
		GAIA::N32 nRet = 0;
		...
		for(__MultiSetType::_datatype x = 0; x < ELEMENT_SIZE; ++x)
		{
			if(!ms.erase(x))
			{
				GTLINE2("MultiSet erase element failed!");
				++nRet;
				break;
			}
		}
