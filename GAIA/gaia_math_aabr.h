...
namespace GAIA
{
	namespace MATH
	{
		...
		public:
			...
			template<typename _ParamDataType> __MyType& operator *= (const GAIA::MATH::VEC2<_ParamDataType>& v){pmin *= v; pmax *= v; return *this;}
			template<typename _ParamDataType> __MyType& operator /= (const GAIA::MATH::VEC2<_ParamDataType>& v){pmin /= v; pmax /= v; return *this;}
			template<typename _ParamDataType> GAIA::MATH::VEC2<_DataType> nearest_point(const GAIA::MATH::VEC2<_ParamDataType>& v) const
			{
				GAIA::MATH::VEC2<_DataType> ret;
				if(v.x < pmin.x)
					ret.x = pmin.x;
				else if(v.x > pmax.x)
					ret.x = pmax.x;
			...
			{
				GAIA::MATH::VEC2<_DataType> nearestpoint = this->nearest_point(v);
