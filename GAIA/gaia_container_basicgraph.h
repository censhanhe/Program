...
namespace GAIA
{
	namespace CONTAINER
	{
		template<typename _DataType, typename _SizeType, typename _SizeIncreaserType> class BasicGraph : public GAIA::Base
		{
		public:
			class Node
			{
			...
			template<typename _KeyType, _SizeType _MaxLinkCount> GINL GAIA::BL navpath_node(const Node& src, const Node& dst, const _SizeType& navtimes, __NodeListType& result) const
			{
				...
				src.enter_traveling();
				{
					...
					for(_SizeType x = 0; x < src.m_links.size(); ++x)
					{
						...
						GAIA_AST(arrlink.size() < arrlink.capacity());
