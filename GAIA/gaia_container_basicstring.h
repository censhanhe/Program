	...
	namespace CONTAINER
	{
		...
		public:
			...
			GINL _SizeType rfind(const _DataType& t, const _SizeType& index) const
			{
				...
				if(index >= this->size())
					return (_SizeType)GINVALID;
				const _DataType* pFinded = GAIA::ALGORITHM::rfind(m_pFront, m_pFront + index - 1, t);
				if(pFinded == GNULL)
					return (_SizeType)GINVALID;
				return pFinded - m_pFront;
			}
			GINL _SizeType rfind(const _DataType* p, const _SizeType& size, const _SizeType& index) const
			{
				...
				if(index >= this->size())
					return (_SizeType)GINVALID;
				const _DataType* pFinded = GAIA::ALGORITHM::rfinds(m_pFront, m_pFront + index - 1, p, size);
				if(pFinded == GNULL)
					return (_SizeType)GINVALID;
				return pFinded - m_pFront;
			}
