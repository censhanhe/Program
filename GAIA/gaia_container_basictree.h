	...
	namespace CONTAINER
	{
		...
		public:
			...
			GINL GAIA::GVOID erase(__Node& n)
			{
				...
				{
					__Node** pFinded = GAIA::ALGORITHM::find(n.m_pParent->m_links.front_ptr(), n.m_pParent->m_links.back_ptr(), &n);
					*pFinded = GNULL;
				}
				m_pool.release(&n);
			}
			...
			GINL _SizeType getlinkcount(const __Node& n) const{return n.m_links.size();}
