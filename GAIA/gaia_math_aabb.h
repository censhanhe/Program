...
namespace GAIA
{
	namespace MATH
	{
		...
		public:
			...
			template<typename _ParamDataType> GAIA::BL operator > (const _ParamDataType& t) const{return !this->operator <= (t);}
			template<typename _ParamDataType> GAIA::BL operator < (const _ParamDataType& t) const{return !this->operator >= (t);}
			template<typename _ParamDataType> const _DataType& operator [] (const _ParamDataType& index) const{GAIA_AST(index < 6); return this->front_ptr()[index];}
			template<typename _ParamDataType> _DataType& operator [] (const _ParamDataType& index){GAIA_AST(index < 6); return this->front_ptr()[index];}
